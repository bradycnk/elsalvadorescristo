...
async function fetchVerseOfTheDay(): Promise<VerseNews> {
  ...
  return {
    title: ogTitle?.trim() || 'Versículo del día',
    description: ogDescription?.trim() || 'Lee la reflexión y el versículo del día en Bible.com.',
    url: 'https://www.bible.com/es/verse-of-the-day'
  };
}

function renderVerseNews(news: VerseNews): void {
...
function renderVerseFallback(errorMessage: string): void {
  const root = document.getElementById('bible-news');
  if (!root) return;
